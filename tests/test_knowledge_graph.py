...
class TestGraphConstruction:
    ...
    def test_build_from_rules(self, sample_rules_list):
        graph = RegulatoryKnowledgeGraph(persist_path="nonexistent.json.enc")
        graph.build_from_rules(sample_rules_list)
        assert graph.is_built()
        assert graph.graph.number_of_nodes() > 0

    def test_agency_nodes_created(self, sample_rules_list):
        graph = RegulatoryKnowledgeGraph(persist_path="nonexistent.json.enc")
        graph.build_from_rules(sample_rules_list)
    ...
    def test_rule_nodes_created(self, sample_rules_list):
        graph = RegulatoryKnowledgeGraph(persist_path="nonexistent.json.enc")
        graph.build_from_rules(sample_rules_list)
        assert graph.graph.has_node("ADR.OR.B.005")
        assert graph.graph.has_node("ORO.FTL.210")

    def test_amc_node_type(self, sample_rules_list):
        graph = RegulatoryKnowledgeGraph(persist_path="nonexistent.json.enc")
        graph.build_from_rules(sample_rules_list)
    ...
    def test_publishes_edges(self, sample_rules_list):
        graph = RegulatoryKnowledgeGraph(persist_path="nonexistent.json.enc")
        graph.build_from_rules(sample_rules_list)
        # EASA should have PUBLISHES edges to rules
        edges = list(graph.graph.out_edges("EASA", data=True))
        assert len(edges) >= len(sample_rules_list)

    def test_cross_reference_edges(self):
        """Rules referencing other rules should create REFERENCES edges."""
        rules = [
            EasaRequirement(
                id="CAT.OP.MPA.150", type="IR",
                text="In accordance with ORO.FTL.210, the operator shall...",
                source_title="Fuel Policy", domain="air-ops", amc_gm_info="Hard Law",
            ),
            EasaRequirement(
                id="ORO.FTL.210", type="IR",
                text="Maximum FDP 13 hours.",
                source_title="FTL", domain="air-ops", amc_gm_info="Hard Law",
            ),
        ]
        graph = RegulatoryKnowledgeGraph(persist_path="nonexistent.json.enc")
...
class TestTraversal:
    ...
    def test_traverse_returns_results(self, sample_rules_list):
        graph = RegulatoryKnowledgeGraph(persist_path="nonexistent.json.enc")
        graph.build_from_rules(sample_rules_list)
    ...
    def test_traverse_nonexistent_node(self, sample_rules_list):
        graph = RegulatoryKnowledgeGraph(persist_path="nonexistent.json.enc")
        graph.build_from_rules(sample_rules_list)
        results = graph.traverse("NONEXISTENT.999", depth=2)
        assert results == []

    def test_get_linked_rules(self, sample_rules_list):
        graph = RegulatoryKnowledgeGraph(persist_path="nonexistent.json.enc")
        graph.build_from_rules(sample_rules_list)
        linked = graph.get_linked_rules("EASA", depth=1)
        assert isinstance(linked, list)

    def test_traverse_depth_limit(self, sample_rules_list):
        graph = RegulatoryKnowledgeGraph(persist_path="nonexistent.json.enc")
        graph.build_from_rules(sample_rules_list)
...
class TestConflictDetection:
    ...
    def test_no_conflicts_default(self, sample_rules_list):
        graph = RegulatoryKnowledgeGraph(persist_path="nonexistent.json.enc")
        graph.build_from_rules(sample_rules_list)
...
class TestStats:
    ...
    def test_get_stats(self, sample_rules_list):
        graph = RegulatoryKnowledgeGraph(persist_path="nonexistent.json.enc")
        graph.build_from_rules(sample_rules_list)
    ...
    def test_get_graph_health(self, sample_rules_list):
        graph = RegulatoryKnowledgeGraph(persist_path="nonexistent.json.enc")
        graph.build_from_rules(sample_rules_list)
    ...
    def test_get_neighbors_summary(self, sample_rules_list):
        graph = RegulatoryKnowledgeGraph(persist_path="nonexistent.json.enc")
        graph.build_from_rules(sample_rules_list)
        neighbors = graph.get_neighbors_summary("ADR.OR.B.005")
        assert isinstance(neighbors, list)

    def test_get_neighbors_nonexistent(self, sample_rules_list):
        graph = RegulatoryKnowledgeGraph(persist_path="nonexistent.json.enc")
        graph.build_from_rules(sample_rules_list)
...
class TestPersistence:
    ...
    def test_persist_and_load(self, sample_rules_list, temp_dir):
